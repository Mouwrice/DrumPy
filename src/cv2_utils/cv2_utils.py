...
def overlay(frame, source_fps, fps, frame_count, video_time_ms, model, width, height):
    ...
    cv2.putText(frame, source_fps_text, text_location, cv2.FONT_HERSHEY_PLAIN,
    ...
    cv2.putText(frame, frame_count_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)

    # Show the time
    time_text = 'Time {:.1f}s'.format(video_time_ms / 1000)
    text_location = (left_margin, row_size * 4)
    cv2.putText(frame, time_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)

    # Show resolution
    resolution_text = 'Resolution {}x{}'.format(width, height)
    text_location = (left_margin, row_size * 5)
    cv2.putText(frame, resolution_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)

    # Show mediapipe model
    model_text = 'Model {}'.format(model.name)
    text_location = (left_margin, row_size * 6)
    cv2.putText(frame, model_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)
